...
def MO_CMA_ES(mu = 50, lambda_ = 100, sigma = 0.1, gen = 30):
    ...
    strategy = cma.StrategyMultiObjective(population, sigma=sigma, mu=mu, lambda_=lambda_)
...
def experiment(function, runs = 10):
    ...
    bigDF.to_csv("experiment_advanced_group"+str(len(group))+"_results.csv")
    with open('experiment_advanced_group'+str(len(group))+'_hof.txt', 'w') as fp:
        for line in bigHOF:
...
def finalBest(hof, iterations = 5, runs = 10):
    ...
    df.to_csv("experiment_advanced_group"+str(len(group))+"_final.csv")
...
bigDF, bigHOF = experiment(MO_CMA_ES)
